...
import XMonad.Actions.SpawnOn -- RCS: to spawn todo and mail to certain windows.  I can't get it work properly though
import XMonad.Util.EZConfig
import XMonad.Util.Run
import XMonad.Hooks.DynamicLog
import XMonad.Actions.Plane
import XMonad.Actions.CycleWS -- RCS: to have comma toggle to last workspace
...
mailLayout = avoidStruts(ResizableTall 1 (3/100) (3/4) [])
termLayout = avoidStruts(Grid)

-- Here we combine our default layouts with our specific, workspace-locked
-- layouts.
myLayouts =
  onWorkspace "7:Chatxunused" chatLayout
  $ onWorkspace "1:Pix" gimpLayout
  $ onWorkspace "3:Mail" mailLayout 
  $ onWorkspace "4:Term" termLayout
  $ defaultLayouts
...
myKeyBindings =
  [
    ...
    , ((myModMask, xK_s), spawn "gnome-power-cmd suspend")
...
-- RCS: I want an easier way to get to the most important workspaces
altKeys =
  [
    xK_m, xK_8, xK_9
    , xK_u, xK_i, xK_o
...
myKeys = myKeyBindings ++
  ...
  -- RCS: Added the more convenient keys to support my laziness
  [
    ((m .|. myModMask, k), windows $ f i)
       | (i, k) <- zip myWorkspaces altKeys
       , (f, m) <- [(W.greedyView, 0), (W.shift, shiftMask)]
  ] ++
...
main = do
  ...
  , logHook = dynamicLogWithPP $ xmobarPP {
    ...
    -- RCS: Added these... not quite right yet...  put in /usr/lib/bin i think
    `additionalKeysP` 
        [-- Lock Screen
    ...
    -- Sleep
    , ("M1-S-'",   spawn "gnome-power-cmd suspend")
    -- Reboot
    , ("M1-S-,",   spawn "gnome-power-cmd reboot")
    -- Deep Sleep
    , ("M1-S-.",   spawn "gnome-power-cmd hibernate")
    -- Death
    , ("M1-S-p",   spawn "gnome-power-cmd shutdown")
    ]
